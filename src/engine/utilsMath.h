...
inline float random01() { return (float)rand() / (float)RAND_MAX; }
...
inline vec3 randomVec(const vec3 &min, const vec3 &max) {
  return vec3(randomFloat(min.x, max.x), randomFloat(min.y, max.y),
              randomFloat(min.z, max.z));
}

inline vec3 randVec() { return randomVec(vec3(-5, -5, -5), vec3(5, 5, 5)); }
...
inline bool isBoxInFrustum(glm::vec4 *frustumPlanes, glm::vec4 *frustumCorners,

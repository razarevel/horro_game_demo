...
struct Renderer {
  ...
  int32_t imagePicking(int32_t x, int32_t y, struct Texture *tex);
...
struct Pipeline *Renderer::createPipeline(const struct PipelineInfo &info_) {
  ...
  if (info_.colorFormat != VK_FORMAT_UNDEFINED)
    pipelineRenderCreateInfo.pColorAttachmentFormats = &info_.colorFormat;
...
struct Texture *Renderer::createImage(const struct TextureInfo &info) {
  ...
  if (format_ == VK_FORMAT_R16G16B16A16_SFLOAT)
    imageSize = info.dimensions.width * info.dimensions.height * 8;
  ...
  if (info.type == MAI::TextureType_2D) {
    ...
    if (format_ == VK_FORMAT_R16G16B16A16_SFLOAT) {
      desc.wrapU = SamplerWrap::Clamp_to_Edge;
      desc.wrapV = SamplerWrap::Clamp_to_Edge;
      desc.wrapW = SamplerWrap::Clamp_to_Edge;
    }

    ctx->createSampler(desc, sampler);
  ...
  if (info.usage != MAI::Sampled_Bit || !defaults.defaultDescriptorPool ||
...
void CommandBuffer::cmdEndRendering(struct Texture *colorTexture) {
  ...
  if (colorTexture)
    ctx->transition_image_layout(
        ...
        VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT,
        VK_PIPELINE_STAGE_2_TRANSFER_BIT, colorTexture->getImage());
  else
    ctx->transition_image_layout(
        VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
...
int32_t Renderer::imagePicking(int32_t x, int32_t y, struct Texture *tex) {
  ...
  VkCommandBuffer commandBuffer = ctx->beginSingleCommandBuffer();
  ...
  vkCmdCopyImageToBuffer(commandBuffer, tex->getImage(),
                         VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, stagingBuf, 1,
                         &region);

  ctx->endSingleCommandBuffer(commandBuffer);

  uint32_t *data = nullptr;
  vmaMapMemory(ctx->allocator, stagingAlloc, (void **)&data);
  ...
  return pickedID;
#endif
  std::cerr << "image pick: [Todo]" << std::endl;
  return -1;
}
...
VkImageUsageFlags getImageUsage(TextureUsage usage) {
  ...
  case TextureUsage::Color_Attachment_Bit:
